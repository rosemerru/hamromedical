...
var s_code_version = "IES|v20170817";

var ps = document.head.getElementsByTagName('script')
...
//Dynamically set s_account
var s_account = sacct_env=='staging'?'chegginciesdev':'chegginciesprod'
var s=s_gi(s_account)

// Instantiate the s object
var s=s_gi(s_account);
s.visitorNamespace = 'easybib';
s.trackingServer = "adobetp.chegg.com";
s.trackingServerSecure = "adobetps.chegg.com";
s.visitor = Visitor.getInstance('3FE7CBC1556605A77F000101@AdobeOrg', {
...
// Mobile App Visitor ID Override
try{
	if(analytics_app_visitor_id!="" && analytics_web_visitor_id == undefined){
		s.visitor = Visitor.getInstance('3FE7CBC1556605A77F000101@AdobeOrg');
//		_satellite.getVisitorId().setMarketingCloudVisitorID(analytics_app_visitor_id);
	}
	} catch(err){	
}
...
s.linkInternalFilters="javascript:,easybib.com,chegg.com,imagineeasy.com,iescholar.com,ieacademy.com,bibme.org,citationmachine.net,citethisforme.com"
s.linkLeaveQueryString=false
s.linkTrackVars='events,prop1,prop2,prop3,prop4,prop6,prop7,prop8,eVar3'
s.linkTrackEvents=''
...
function s_doPlugins(s) {
	...
	//s.eVar75 = 'v110316a|' + s.version + '|'
	//s.eVar75 += typeof s.visitor != 'undefined' ? s.visitor.version : 'N/A'
	...
	// User Agent Tracking
	s.prop18 = s.eVar60 = 'D=User-Agent'
	...
	//Page Name/Page URL Tracking
	s.eVar5 = document.location.host + document.location.pathname;
	s.prop6 = s.eVar4 = document.location.search;

	// Previous Page Name
	s.eVar6 = s.getPreviousValue(s.pageName,'gpv_v5','');  
	
	// Custom Page Name
	s.eVar7 = s.pageName;

	// Set Time Parting Variables 
	s.eVar12 = s.getTimeParting('n','-6');	
	
	// Marketing Cloud Visitor ID
	s.prop20 = 'D=mid'	
	
	// RSID(s)
	s.contextData['cDrsid'] = s.account;

	// Determine if the Visitor API is enabled
	...
	if(s.events){
		...
		s.linkTrackEvents=s.apl(s.linkTrackEvents,ea,',',0);		
	}	
...
/* Function - write combined cookies v 0.42 */
if(!s.__ccucw){
    s.c_wr = s.c_w;
    s.__ccucw = true;
    function c_w(k, v, e){
...
/* Utility: sessionClean 0.11 */
s.sessionClean=new Function("v",""
+"if(typeof v!='undefined'){var s=this;y=s.sessionExcludeStrings.spli"
+"t('|');for(var x=0;x<y.length;x++){z=new RegExp(y[x]+'([^\?\&]*)');"
+"v=v.replace(z,'');}return v}");
